...
def ALGP_clustering_BM(sim, dmat): 
    ...
    np.fill_diagonal(sim, 0)
    seuil=np.unique(sim)
    partition = [PropagationClustering().fit_predict(sparse.csr_matrix(sim > th)) for th in seuil[:-1]  ]
...
def ConnectedComponents_clustering_BM(sim, dmat):
    ...
    np.fill_diagonal(sim, 0)
    seuil=np.unique(sim)
    partitions = [get_connected_components(sparse.csc_matrix(sim > t)) for t in seuil[:-1]]
    sil = [silhouette_score(d, p, metric="precomputed") if (len(set(p)) > 1 and len(set(p))<len(sim)) else 0 for p in partitions]
    return partitions[np.argmax(np.array(sil))]
